...
export const FromToken = () => {
	...
	useEffect(() => {
		try {
			...
			if (poolList && poolList.length > 0 && inputValue) {
				const valueB = findTokenByDenom(from.token.denom)
				const multipliedValues =
					Number.parseFloat(inputValue) * Number.parseFloat(valueB.toString())
				const roundedValues = multipliedValues.toFixed(2) // Round to 2 decimal places
				setDollarValue(roundedValues)
			}
	...
	const customStyles: ChakraStylesConfig<TokenWithBalance, true, GroupBase<TokenWithBalance>> = {
		...
		menuList: (provided: SystemStyleObject) => ({
			...provided,
			"&::-webkit-scrollbar": {
				background: useColorModeValue("rgba(160,160,160,0.1)", "rgba(160,160,160,0.1)"),
				borderRadius: "4px",
				width: "12px"
			},
		...
		>
			...
			>
				<Flex align="center" gap={2} justify="start" maxW={{ sm: "2xs" }} w="full">
					...
					<NumericFormat
						_dark={{ _focus: { bg: "rgba(143, 143, 143, 0.05)" } }}
						_disabled={{}}
							...
							{from && (
								...
								>
									<NumericFormat
										...
										fontWeight="bold"
										minH="3rem"
										pl="2.5rem" // Add padding to the left to make space for the dollar value
