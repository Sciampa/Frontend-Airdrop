...
import { convertMicroDenomToDenom, initialUSDCToken } from "utils/tokens/helpers"
...
export const ToToken = () => {
	...
	const [dollarValueLoading, setDollarValueLoading] = useState(false)
	const [poolList, setPoolList] = useState<TPool[]>([])
	const [previousPrice, setPreviousPrice] = useState(tokenToTokenPrice.price)

	useEffect(() => {
		try {
			const fetchPoolList = async () => {
				const poolListResponse = await fetch("https://api.electronprotocol.io/poolList")
				const poolListResult = await poolListResponse.json()
				setPoolList(poolListResult.pools)
			}

			// Function to find token price in the pool list
			const findTokenByDenom = (denomValue: string) => {
				for (const pool of poolList) {
					const { token1, token2 } = pool.liquidity
					if (token1.denom === denomValue) {
						return token1.tokenPrice
					} else if (token2.denom === denomValue) {
						return token2.tokenPrice
					}
				}
			...
			// Ensure fetchPoolList runs and updates poolList
			if (
				tokenToTokenPrice.price !== 0 &&
				!dollarValueLoading &&
				tokenToTokenPrice.price !== previousPrice
			) {
				setDollarValueLoading(true)
				void fetchPoolList()
				setPreviousPrice(tokenToTokenPrice.price)
			}

			// Update the calculation logic to include the initialUSDCToken
			if (poolList && poolList.length > 0 && tokenToTokenPrice) {
				const valueB = findTokenByDenom(to.token.denom) || initialUSDCToken.tokenPrice
				const multipliedValues =
					Number.parseFloat(tokenToTokenPrice.price) * Number.parseFloat(valueB.toString())
				const roundedValues = new BigNumber(multipliedValues)
				const decimalPlaces = to.token.decimal

				const result = roundedValues.dividedBy(new BigNumber(10).pow(decimalPlaces)).toFixed(2) // Adjust decimal places as needed

				setDollarValueLoading(false)
				setDollarValue(result.toString())
			}
		} catch {
			setDollarValueLoading(false)
		}
		...
		// eslint-disable-next-line react-hooks/exhaustive-deps
	}, [tokenToTokenPrice, poolList, previousPrice, to.token.denom])
	...
	useEffect(() => {
		try {
			...
			const valueB = findTokenByDenom(to.token.denom)
			...
			if (poolList && poolList.length > 0 && tokenToTokenPrice) {
				...
				const roundedValues = new BigNumber(multipliedValues)
				const decimalPlaces = 2 // Set to 2 decimal places
					...
					{({ isOpen, onClose }) => (
						<>
							<Flex align="space-between" direction="row" gap={3} pt={6} w="full">
								<MenuButton
									_dark={{ bg: "rgba(143, 143, 143, 0.05)" }}
									as={Button}
								...
								{to && (
									...
									>
										...
										<NumericFormat
											_dark={{ _focus: { bg: "rgba(143, 143, 143, 0.05)" }, color: "white" }}
											_disabled={{}}
										...
										{dollarValue !== null && (
											<Text
												...
												mr="0.4rem"
												position="relative"
												right={0} // Adjust this value for proper alignment
												fontWeight="bold"
												fontSize={{ base: "xs", md: "14" }}
												color={useColorModeValue("gray.500", "gray.300")} // Adjust color for contrast
												pointerEvents="none" // Prevent interaction with this text
											>
												{`$${dollarValue}`}
											</Text>
										)}
