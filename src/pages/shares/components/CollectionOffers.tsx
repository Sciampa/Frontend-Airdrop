...
import { useContractOffers } from "../hooks/query/useListing"
import {
	Divider,
	Flex,
	HStack,
...
const CollectionOffers: React.FC<CollectionDescriptionProps> = ({ collection }) => {
	const { marketAddress } = collection
	const [contractList, isLoading, error] = useContractOffers(marketAddress)
	...
	const sortedSwaps = swaps
		...
		.sort((b, a) => Number(a.price) - Number(b.price))
	const firstFourSwaps = sortedSwaps.slice(0, 4)
