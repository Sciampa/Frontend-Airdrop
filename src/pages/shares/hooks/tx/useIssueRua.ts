...
export const useExecuteContractMsg = (issuerContract: string) => {
	...
	const fetchAvailableSwaps = async () => {
		...
		try {
			const result = await client.queryContractSmart(issuerContract, queryMessage)
			...
			console.log("Extracted swaps:", swaps)
			return swaps
		} catch (error) {
	...
	return useMutation(
		...
		async () => {
			...
			try {
				return await client.execute(address!, issuerContract, message, "auto", undefined, funds) // Use issuerContract here
			} catch (error) {
