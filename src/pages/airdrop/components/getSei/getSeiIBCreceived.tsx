...
const SeiIBCreceived = () => {
	const { address, isWalletConnected } = useChain("sei")
		...
		const fetchPercentage = async (): Promise<void> => {
			try {
				const response = await axios.get<AddressData[]>(
					"https://raw.githubusercontent.com/Electron-Protocol/airdrop/main/Sei/Sei_percentage_IBCreceived.json"
				)
