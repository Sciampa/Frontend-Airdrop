...
const SeiIBCtransfer = () => {
	const { address, isWalletConnected } = useChain("stride")
		...
		const fetchPercentage = async (): Promise<void> => {
			try {
				const response = await axios.get<AddressData[]>(
					"https://raw.githubusercontent.com/Electron-Protocol/airdrop/main/Stride/Stride_counts_IBC_percentage.json"
				)
