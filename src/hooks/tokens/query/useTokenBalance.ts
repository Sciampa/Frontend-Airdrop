...
import { type StargateClient } from "@cosmos-kit/core/node_modules/@cosmjs/stargate/build/stargateclient"
...
export const useTokenBalance = (denom: string) => {
	const { address, getSigningStargateClient, isWalletConnected } = useChain(
...
export const useMultipleTokenBalance = (tokens: Token[]) => {
	const { address, getStargateClient, isWalletConnected } = useChain(
		import.meta.env.VITE_NEUTRONNETWORK
	)
