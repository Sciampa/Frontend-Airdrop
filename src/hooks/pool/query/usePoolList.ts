...
export const poolListUrl = `https://raw.githubusercontent.com/Electron-Protocol/Assetlist/main/assets/${
...
export const usePoolFromListQueryById = ({ poolId }: { poolId: number }) => {
	...
	const requestedPool = useMemo(() => {
		...
		return poolsList?.poolsWithAPR.find(
			(pool: { pool: { poolId: number } }) => pool.pool.poolId === poolId
		)
