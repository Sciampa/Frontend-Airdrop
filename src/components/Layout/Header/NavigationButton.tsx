...
export type NavigationButtonProps = ButtonProps & {
	...
	navId: number
	onClick?: () => void
	subLinks?: Record<string, { icon: ReactElement; isDisabled?: boolean; url: string }>
	url: string
}
					...
					{subLinks && (
						<AccordionPanel p={0}>
							{Object.entries(subLinks).map(([key, subLink]) => {
								const isSubLinkDisabled = subLink.isDisabled
								return (
									<Link
										key={key}
										style={{
											opacity: isSubLinkDisabled ? 0.5 : 1,
											pointerEvents: isSubLinkDisabled ? "none" : "all",
											position: "relative"
										}}
										to={isSubLinkDisabled ? "" : url + subLink.url}
									>
										<Button
											...
											color="white"
											filter={
												isActive && pathname === url + subLink.url
